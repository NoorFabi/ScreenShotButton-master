...
//        notificationManager.notify(notificationId, createRunningNotification(isCapturing))
            ...
            for (nTry in 1..3) {
                ...
                try {
                    ...
                    launch(Dispatchers.Main) {
                        performOCRAndShowResult(context, captureResult.documentUri)
                    }
    ...
    private suspend fun performOCRAndShowResult(context: Context, documentUri: Uri?) {
        documentUri?.let { uri ->
            withContext(Dispatchers.IO) {
                try {
                    ...
                    inputStream?.use { stream ->
                        ...
                        val ocrResult = OCRUtil.performOCR(context, bitmap)

                        // Log the OCR result
                        Log.d("OCR_RESULT", "OCR Result: $ocrResult")

                        // Display a Toast message indicating OCR completion
                        withContext(Dispatchers.Main) {
                            val message = if (ocrResult.isNotEmpty()) {
                    ...
                    // Display a Toast message for OCR failure
                    withContext(Dispatchers.Main) {
    ...
    private fun processTextBlocks(textBlocks: SparseArray<TextBlock>): String {
        ...
        for (index in 0 until textBlocks.size()) {
            val textBlock: TextBlock = textBlocks.valueAt(index)
